...
Sistema híbrido de 3 niveles:
  1. Motor de reglas con palabras clave (siempre disponible, sin internet)
  2. Groq API con modelo LLM (enriquece la respuesta si hay API key)
  3. Fallback: respuesta básica si no se detecta el tema

El chatbot detecta el tipo de trabajo (baño, cocina, electricidad, etc.)
y recomienda los materiales necesarios con cantidades estimadas.
"""
import json
import logging
import re
...
# Base de conocimiento: cada tipo de trabajo tiene sus materiales típicos
KNOWLEDGE_BASE = {
    'baño': {
...
# Sinónimos para que el chatbot entienda variantes de cada tema
SYNONYMS = {
...
def _normalize(text):
    """Quita tildes y puntuación para poder comparar palabras."""
    text = text.lower()
...
class ConstructionChatbot(models.AbstractModel):
    ...
    def _detect_topics(self, message_normalized):
        """Detecta los temas mencionados en el mensaje del usuario."""
        topics = []
    ...
    def _build_rules_response(self, topics):
        """Genera la respuesta con la lista de materiales recomendados."""
        if not topics:
            return {
                'tipo': 'desconocido',
                'mensaje': 'No he podido identificar el tipo de trabajo. '
                           'Prueba con: baño, cocina, electricidad, pintura, '
                           'suelo, tabique, fontanería o aislamiento.',
                'materiales': [],
                'fuente': 'reglas',
            }
    ...
    def _call_groq_api(self, message, rules_response):
        """Llama a la API de Groq (LLM) para enriquecer la respuesta.
        Si no hay API key configurada o falla la llamada, devuelve None."""
    ...
    def process_message(self, message):
        ...
        1. Detecta el tema por palabras clave
        2. Genera lista de materiales recomendados
        3. Opcionalmente enriquece con IA (Groq LLM)"""
